...
The RMS Titanic was a British passenger liner that sank in 1912. Over half of the 2224 passengers lost their lives, making the sinking of the Titanic one of the deadliest peacetime maritime disasters. A number of factors contributed to survivorship.  The attached .csv files document the passengers of this ship, some of their information, and whether they survived.
...
## LOAD DATA
```{r load data}
train <- read.csv('C:/Users/PC - Main/Desktop/Data Science/train.csv', header=T, na.strings=c("")) # 891 obs. of 12 variables
test <- read.csv('C:/Users/PC - Main/Desktop/Data Science/test.csv', header=T, na.strings=c("")) # 418 obs. of 11 variables
```
...
## MISSING VALUES
Identify any or all missing values in the data-set. Normally, some missing values can be omitted from the data-set depending on the size or by how easy they are to rectify. First, I wanted to take a look visually:
...
The Cabin is a variable that can NOT be easy to rectify as a result of it being a string and the number of missing values. Thus, Cabin will be omitted from the model. All other NA variables can be remedied by using the median or max value within each variables column.
...
Created six categories for Title: (1)Officer (2) Royalty (3) Miss (4) Mrs (5) Mr (6) Master. This will allow me to interpret better to see if the passengers title had an effect on their survival.
...
Therefore, as a baseline, passengers from the training data-set had a ~0.4 chance of surviving the sinking of the Titanic. I want my model to NOT predict lower than ~0.4.
